...
async def get_scans(
    ...
    try:
        ...
        logger.error("[history] get_scans failed", exc_info=True)
...
async def get_scans_with_anomalies(
    ...
    """
    Returns scans with anomalies embedded — single query, no N+1.
    Used by results.tsx to replace the getScans + N×getScan pattern.
    """
    db = get_db()
    try:
        result = (
        ...
        logger.error("[history] get_scans_with_anomalies failed", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error.")
...
async def get_scan(scan_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error("[history] get_scan failed scan_id=%s", scan_id, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error.")
...
async def acknowledge_scan(scan_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.info("[history] scan acknowledged scan_id=%s user=%s", scan_id, current_user["sub"])
        return {"success": True, "acknowledged_at": now}
    except HTTPException:
        raise
    except Exception:
        logger.error("[history] acknowledge failed scan_id=%s", scan_id, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error.")
